...
;
; CALLING SEQUENCE:
;     result = mc_avehdrs(hdrs,TIME_OBS=time_obs,POSANGLE=posangle,$
...
;                  gethdrinfo
;     2012-08-06 - Added fixed keywords to be consistent with Spextool code.
;-
function mc_avehdrs,hdrs,TIME_OBS=time_obs,POSANGLE=posangle,HA=ha,$
...
;  Check parameters
  ...
  if n_params() lt 1 then begin
     ...
     print, 'Syntax -  result = mc_avehdrs(hdr,TIME_OBS=time_obs,$'
...
;  Check to see if the times are bracketed around midnight.
     ...
     newTIME_OBS = string(fix(result[0]),FORMAT='(i2.2)')+':'+$
                   string(fix(result[1]),FORMAT='(i2.2)')+':'+ $
                   strcompress(result[2],/RE)
...
;  Check to see if it is centered around 0.
  ...
  if n_elements(HA) ne 0 then begin
     ...
     for i = 0, n-1 do values[*,i] = $
     double(strsplit(hdrs[i].vals.(zh),':',/EXTRACT))
  ...
  if gotimeobs then begin
     ...
     vals.(zt) = newTIME_OBS
     coms.(zt) = ' Average '+strtrim(newhdr.coms.(zt),2)
  ...
  if goposangle then begin
     ...
     vals.(zp) = newPOSANGLE
     coms.(zp) = ' Average '+strtrim(newhdr.coms.(zp),2)

     if posangle ne 'POSANGLE' then begin

        vals = create_struct(vals,'POSANGLE',newPOSANGLE)
        coms = create_struct(coms,'POSANGLE',' Average '+ $
                             strtrim(newhdr.coms.(zp),2))
  ...
  if goha then begin
     ...
     vals.(zh) = newHA
     coms.(zh) = ' Average '+strtrim(newhdr.coms.(zh),2)
     
     if ha ne 'HA' then begin

        vals = create_struct(vals,'HA',newHA)
        coms = create_struct(coms,'HA',' Average '+strtrim(newhdr.coms.(zh),2))
  ...
  if goairmass then begin
     ...
     vals.(za) = newAIRMASS
     coms.(za) = ' Average '+strtrim(newhdr.coms.(za),2)

     if airmass ne 'AIRMASS' then begin
        
        vals = create_struct(vals,'AIRMASS',newAIRMASS)
        coms = create_struct(coms,'AIRMASS',' Average '+strtrim(newhdr.coms.(za),2))

     endif

  endif

  newhdr = {vals:vals,coms:coms}
...
;
;
;    vals = (i eq 0) ? create_struct('POSANGLE',val):create_struct(vals,key,val)
