...
;
; CALLING SEQUENCE:
;     result = mc_mkfullpath(path,files,INDEX=index,FILENAME=filename,NI=ni,$
...
;
; EXAMPLE:
;     result = mc_mkfullpath('/scr1/data/',['3','4','5'],/INDEX,NI=4,$
;                            PREFIX='spc',SUFFIX='.fits'/EXIST)
;
; MODIFICATION HISTORY:
;     2000-08-16 - Written by M. Cushing, Institute for Astronomy, UH
;     2013-06-05 - Now uses the file_info program instead findfile.
;-
function mc_mkfullpath,path,files,INDEX=index,FILENAME=filename,NI=ni,$
                       PREFIX=prefix,SUFFIX=suffix,EXIST=exist,$
                       WIDGET_ID=widget_id,CANCEL=cancel
...
;  Check parameters
  ...
  if n_params() lt 2 then begin
     ...
     cancel = 1
     print, 'Syntax - result = mc_mkfullpath(path,files,INDEX=index,$'
  ...
  cancel = mc_cpar('mc_mkfullpath',path,1,'Path',7,0)
  if cancel then return,-1
  cancel = mc_cpar('mc_mkfullpath',files,2,'Files',7,[0,1])
  if cancel then return,-1
...
;  Now build the paths string array.
     ...
     for i = 0, n_elements(files) - 1 do begin
        ...
        if keyword_set(EXIST) then begin
           ...
           name = mc_cfile(name,WIDGET_ID=widget_id,CANCEL=cancel)
           if cancel then return,-1
...
;  Check to see if the files exist if requested.
  ...
  if keyword_set(EXIST) then begin
     ...
     for i = 0, nimages - 1 do begin
        ...
        file = mc_cfile(fullpaths[i],WIDGET_ID=widget_id,CANCEL=cancel)
        if cancel then return, -1
