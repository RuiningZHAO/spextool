...
;
; CALLING SEQUENCE:
;     result = mc_slittrans(width,height,fwhm,xoffset,yoffset,CANCEL=cancel)
;
; INPUTS:
;     width   - Width of slit.
...
;
; EXAMPLE:
;     result = mc_slittrans(0.3,15,0.6,0,0,CANCEL=cancel)
;     
...
;-
function mc_slittrans,width,height,fwhm,xoffset,yoffset,CANCEL=cancel

cancel = 0

if n_params() ne 5 then begin
    
    print, 'Syntax - result = mc_slittrans(width,height,fwhm,xoffset,yoffset,$'
    print, '                               CANCEL=cancel)'
    cancel = 1
    return, -1
...
cancel = mc_cpar('mc_slittrans',width,1,'Width',[2,3,4,5],0)
if cancel then return,-1
cancel = mc_cpar('mc_slittrans',height,2,'Height',[2,3,4,5],0)
if cancel then return,-1
cancel = mc_cpar('mc_slittrans',fwhm,3,'FWHM',[2,3,4,5],0)
if cancel then return,-1
cancel = mc_cpar('mc_slittrans',xoffset,4,'Xoffset',[2,3,4,5],0)
if cancel then return,-1
cancel = mc_cpar('mc_slittrans',yoffset,5,'Yoffset',[2,3,4,5],0)
if cancel then return,-1
