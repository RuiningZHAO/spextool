...
;
; CALLING SEQUENCE:
;     result = mc_cfld(field_id,type,EMPTY=empty,CANCEL=cancel)
;
; INPUTS:
...
;
; KEYWORD PARAMETERS:
;     EMPTY  - If set, mc_cfld will return and error if the field is empty.
...
;-
function mc_cfld,field_id,type,EMPTY=empty,CANCEL=cancel
...
if n_params() lt 2 then begin
    ...
    print, 'Syntax -  result = mc_cfld(field_id,type,EMPTY=empty,CANCEL=cancel)'
    cancel = 1
    return, -1

endif
cancel = mc_cpar('mc_cfld',field_id,1,'Field_id',[2,3,4,5],1)
if cancel then return,-1
cancel = mc_cpar('mc_cfld',type,2,'Type',[2,3,4,5],0)
if cancel then return,-1
