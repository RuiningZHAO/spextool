...
;
; CALLING SEQUENCE:
;     mc_findpeaks,profiles,npeaks,doorders,positions,apsign,AUTO=auto,$
...
;     doorders - An array [norders] of 1s and 0s.  If 0, the mc_findpeaks
;                skips the order
;     
; OPTIONAL INPUTS:
;     None
...
;-
pro mc_findpeaks,profiles,npeaks,doorders,positions,apsign,AUTO=auto, $
                 GUESS=guess,FIXED=fixed,CANCEL=cancel
  ...
  if n_params() lt 3 then begin
     ...
     print, 'Syntax - mc_findpeaks,profiles,npeaks,doorders,positions,apsign,$'
     print, '                      AUTO=auto,GUESS=guess,FIXED=fixed,$'
     print, '                      CANCEL=cancel'
     cancel = 1
     return
     
  endif
  cancel = mc_cpar('mc_findpeaks',profiles,1,'Profiles',8,[0,1])
  if cancel then return
  cancel = mc_cpar('mc_findpeaks',npeaks,2,'Npeaks',[2,3,4,5],0)
  if cancel then return
  cancel = mc_cpar('mc_findpeaks',doorders,3,'Doorders',[2,3,4,5],[0,1])
  if cancel then return
...
;            read, re
  ...
  if keyword_set(AUTO) then begin
     ...
     for i = 0, norders-1 do begin
        ...
        profile = profiles.(i)
        x = profile[*,0]
        y = abs(profile[*,1]-median(profile[*,1],/EVEN))
