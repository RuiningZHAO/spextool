...
;
; CALLING SEQUENCE:
;     result = mc_normspecflat,flat,edgecoeffs,xranges,slith_arc,nxgrid,nygrid,$
;                              oversamp,ybuffer,IVAR=ivar,OVAR=ovar,$
;                              RMS=rms,UPDATE=update,WIDGET_ID=widget_id,CANCEL=cancel
;
; INPUTS:
...
;                  straightened order (see mc_fiterpolate)
...
;
; KEYWORD PARAMETERS:
;     IVAR      - The input variance image.
;     OVAR      - The output variance image.
...
;     the spatial dimension.  Then uses mc_fiterpolate to fit a smooth
...
;     smooth image.  At no time are the raw data resampled.
;
; EXAMPLE:
;
...
;                  mkspatmap and mc_fiterpolate.  
;     2003-04-10 - Modified the rms computing to use robuststats.
;     2005-04-10 - Added model keyword
;     2014-05-27 - Added the IVAR and OVAR keywords.
;-
function mc_normspecflat,flat,edgecoeffs,xranges,slith_arc,nxgrid,nygrid,$
                         oversamp,ybuffer,RMS=rms,IVAR=ivar,OVAR=ovar,$
                         MODEL=model,UPDATE=update,WIDGET_ID=widget_id,CANCEL=cancel
...
;  Check parameters
  ...
  if n_params() lt 8 then begin
     ...
     print, 'Syntax - result = mc_normspecflat(flat,edgecoeffs,xranges,$'
     print, '                                  slith_arc,nxgrid,nygrid,$'
     print, '                                  oversamp,ybuffer,RMS=rms,$'
     print, '                                  IVAR=ivar,OVAR=ovar,$'
     print, '                                  MODEL=model,UPDATE=update,$'
  ...
  cancel = mc_cpar('mc_normspecflat',flat,1,'Flat',[2,3,4,5],2)
  if cancel then return,-1
  cancel = mc_cpar('mc_normspecflat',edgecoeffs,2,'Edgecoeffs',[2,3,4,5],[2,3])
  if cancel then return,-1
  cancel = mc_cpar('mc_normspecflat',xranges,3,'Xranges',[2,3,4,5],[1,2])
  if cancel then return,-1
  cancel = mc_cpar('mc_normspecflat',slith_arc,4,'Slith_arc',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_normspecflat',nxgrid,5,'Nxgrid',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_normspecflat',nygrid,6,'Nygrid',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_normspecflat',oversamp,7,'Oversamp',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_normspecflat',ybuffer,8,'Ybuffer',[2,3,4,5],0)
  if cancel then return,-1
...
;  Initialize arrays.
  ...
  if arg_present(IVAR) then ovar = fltarr(ncols,nrows)*!values.f_nan
...
;  Normalize each order.
  ...
  for i =0,norders-1 do begin
     ...
     map = mc_rectorder(flat,edgecoeffs[*,*,i],xranges[*,i],slith_arc,1,2,$
...
;  Fiterpolate the map
     ...
     map = mc_fiterpolate(median(map,5),nxgrid,nygrid)
...
;  Determine pixel to arcsecond transformation
        ...
        ovar[j+start,round(botedge[j]+ybuffer):round(topedge[j]-ybuffer)] = $
           ivar[j+start,round(botedge[j]+ybuffer):round(topedge[j]-ybuffer)]/$
           nslit
