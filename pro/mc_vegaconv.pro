...
;
; CALLING SEQUENCE:
;     mc_vegaconv,wave,flux,wvega,fcvega,fnvega,wmin,wmax,wline,kernel,scale,$
...
;                  plot titles
;-
pro mc_vegaconv,wave,flux,wvega,fcvega,fnvega,wmin,wmax,wline,kernel,scale,$
...
;  Check parameters
  ...
  if n_params() lt 7 then begin
     ...
     print, 'Syntax - mc_vegaconv,wave,flux,wvega,fcvega,fnvega,wmin,wmax,$'
     print, '                     wline,kernel,scale,maxdev,rmsdev,WID=wid,$'
     print, '                     STEPS=steps,TWF=twf,CANCEL=cancel'
     cancel = 1
     
  endif
  cancel = mc_cpar('mc_vegaconv',wave,1,'Wave',[1,2,3,4,5],1)
  if cancel then return
  cancel = mc_cpar('mc_vegaconv',flux,2,'Flux',[1,2,3,4,5],1)
  if cancel then return
  cancel = mc_cpar('mc_vegaconv',wvega,3,'Wvega',[1,2,3,4,5],1)
  if cancel then return
  cancel = mc_cpar('mc_vegaconv',fcvega,4,'Fcvega',[1,2,3,4,5],1)
  if cancel then return
  cancel = mc_cpar('mc_vegaconv',fnvega,5,'Fnvega',[1,2,3,4,5],1)
  if cancel then return
  cancel = mc_cpar('mc_vegaconv',wmin,6,'Wmin',[1,2,3,4,5],0)
  if cancel then return
  cancel = mc_cpar('mc_vegaconv',wmax,7,'Wmax',[1,2,3,4,5],0)
  if cancel then return
...
;  Require the number of model/data points to be even.

  if ndata mod 2 eq 1 then begin

     zdata = [zdata,zdata[ndata-1]+1]
     ndata = ndata+1
     
  endif
  
  if nmodl mod 2 eq 1 then begin

     zmodl = [zmodl,zmodl[nmodl-1]+1]
     nmodl = nmodl+1
...
; --- Apply a tapered window function
  ...
  plot,freq,float(fftmodl*fftmodl),/XSTY,XRANGE=[-500,500]
  oplot,freq,fitfft1,COLOR=2
...
;  Compute the ratio and get the residuals
  ...
  mc_moments,ratio,mean,var,rmsdev,/SILENT
  print, ''
...
;  Plot the results
  ...
  if keyword_set(STEPS) or n_elements(WID) ne 0 then begin
     ...
     plot,wldata,fldata,/xsty,/ysty,xrange=[wmin,wmax],yrange=[-0.4,0.1],$
          psym=10,CHARSIZE=1.5,XTITLE='!5!7k!5 (!7l!5m)',$
     ...
     xyouts,0.15,0.3,'!5Max Deviation: '+$
            string(maxdev*100.,format='(f4.2)')+'%',/NORM,CHARSIZE=1.5
     xyouts,0.15,0.25,'!5RMS Deviation: '+$
            string(rmsdev*100.,format='(f4.2)')+'%',/NORM,CHARSIZE=1.5
     
     xyouts, 0.75,0.4,'!5A0 V',/NORM,CHARSIZE=1.5
     xyouts, 0.75,0.35,'!5Vega',/NORM,color=2,CHARSIZE=1.5
     xyouts, 0.75,0.3,'!5Convolved Vega',/NORM,color=3,CHARSIZE=1.5
     xyouts, 0.75,0.25,'!5Residuals',/NORM,color=4,CHARSIZE=1.5
