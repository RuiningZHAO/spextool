...
;
; CALLING SEQUENCE:
;     result = mc_crange(value,range,name,KGT=kgt,KGE=kge,KLT=klt,KLE=kle,$
...
;
; EXAMPLE:
;     result = mc_crange(5,[3,8],'Number',/KGT,/KLT,CANCEL=cancel)
;
; MODIFICATION HISTORY:
;     2000-09-06 - written by M. Cushing, Institute for Astronomy, UH
;-
function mc_crange,value,range,name,KGT=kgt,KGE=kge,KLT=klt,KLE=kle,$
                ODD=odd,EVEN=even,WIDGET_ID=widget_id,CANCEL=cancel
...
if n_params() lt 3 then begin
    ...
    print, 'Syntax -  result = mc_crange(value,range,name,KGT=kgt,KGE=kge,$'
...
cancel = mc_cpar('mc_crange',value,1,'Values',[1,2,3,4,5],[0,1])
if cancel then return,-1
cancel = mc_cpar('mc_crange',range,2,'Range',[1,2,3,4,5],[0,1])
if cancel then return,-1
cancel = mc_cpar('mc_crange',name,3,'Name',7,0)
if cancel then return,-1 
