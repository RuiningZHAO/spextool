...
;
; CALLING SEQUENCE:
;     mc_parangle, HA, DEC, lat, eta, [za], CANCEL=cancel
;
; INPUTS:
...
;
;-
pro mc_parangle, HA, DEC, lat, eta, za, CANCEL=cancel

  cancel = 0
  
  if n_params() lt 3 then begin
     
     print, 'Syntax -  mc_parangle,HA,DEC,lat,eta,za,CANCEL=cancel'
     cancel = 1
     return
     
  endif
  
  cancel = mc_cpar('mc_parangle',HA,1,'HA',[2,3,4,5],0)
  if cancel then return
  cancel = mc_cpar('mc_parangle',DEC,2,'DEC',[2,3,4,5],0)
  if cancel then return
  cancel = mc_cpar('mc_parangle',lat,3,'lat',[2,3,4,5],0)
  if cancel then return
