...
;
; CALLING SEQUENCE:
;     result = mc_robustsg(x,y,width,thresh,eps,DEGREE=degree,$
;                          IGOODBAD=igoodbad,OGOODBAD=ogoodbad,PLT=plt,$
;                          RMS=rms,CANCEL=cancel)
;
; INPUTS:
...
;                - Added PLT keyword.
;
function mc_robustsg,x,y,width,thresh,eps,DEGREE=degree,IGOODBAD=igoodbad,$
...
;  Check parameters
  ...
  if n_params() lt 5 then begin
     ...
     print, 'Syntax -  result = mc_robustsg(x,y,width,thresh,eps,$'
     print, '                               DEGREE=degree,IGOODBAD=igoodbad,$'
     print, '                               OGOODBAD=ogoodbad,PLT=plt,RMS=rms,$'
     print, '                               CANCEL=cancel'
     return, -1
     
  endif

  cancel = mc_cpar('mc_robustsg',x,1,'X',[1,2,3,4,5],1)
  if cancel then return, -1
  cancel = mc_cpar('mc_robustsg',y,2,'Y',[1,2,3,4,5],1)
  if cancel then return, -1
  cancel = mc_cpar('mc_robustsg',width,3,'Width',[1,2,3,4,5],0)
  if cancel then return, -1
  cancel = mc_cpar('mc_robustsg',thresh,4,'Thresh',[1,2,3,4,5],0)
  if cancel then return, -1
  cancel = mc_cpar('mc_robustsg',eps,5,'Eps',[1,2,3,4,5],0)
  if cancel then return, -1
...
;  Do a first pass fit to the data.
  ...
  test = yy - goodyy
  mc_moments, test,mean,dummy,sig,/SILENT
...
;  If it doesn't find any outliers then go back, if not continue.
  ...
  if count_good eq count_initgood or count_good eq 0 then goto, cont1
