...
;
; CALLING SEQUENCE:
;     coeff = mc_aptotrace1d(edgecoeffs,slith_arc,pos_arc,xranges,step,$
;                            fitdegree,nrows,WID=wid,OPLOT=oplot,CANCEL=cancel)
;
; INPUTS:
...
;-
function mc_aptotrace1d,edgecoeffs,slith_arc,pos_arc,xranges,step,fitdegree, $
                        nrows,WID=wid,OPLOT=oplot,CANCEL=cancel
...
;  Check parameters
  ...
  if n_params() lt 7 then begin
     ...
     print, 'Syntax - result = mc_aptotrace1d(edgecoeffs,slith_arc,pos_arc,$'
     print, '                                 xranges,step,fitdegree,nrows,$'
     print, '                                 WID=wid,OPLOT=oplot,$'
     print, '                                 CANCEL=cancel)'
     cancel = 1
     return, -1
  ...
  cancel = mc_cpar('mc_aptotrace1d',edgecoeffs,1,'Edge Coefficients',[2,3,4,5], $
                [2,3])
  if cancel then return, -1
  cancel = mc_cpar('mc_aptotrace1d',slith_arc,2,'Slith_arc',[2,3,4,5],0)
  if cancel then return, -1
  cancel = mc_cpar('mc_aptotrace1d',pos_arc,3,'Pos_arc',[2,3,4,5],[1,2])
  if cancel then return, -1
  cancel = mc_cpar('mc_aptotrace1d',xranges,4,'Xranges',[2,3,4,5],[1,2])
  if cancel then return, -1
  cancel = mc_cpar('mc_aptotrace1d',step,5,'Step',[2,3,4,5],0)
  if cancel then return, -1
  cancel = mc_cpar('mc_aptotrace1d',fitdegree,6,'Fitdegree',[2,3,4,5],0)
  if cancel then return, -1
  cancel = mc_cpar('mc_aptotrace1d',nrows,7,'Nrows',[2,3,4,5],0)
  if cancel then return, -1
  
;  Get setup info.
  
  mc_arrinfo,edgecoeffs,ndimen,dimen
...
;  Now fit the peak locations with a polnomial.
     ...
     for k = 0,naps-1 do begin
        ...
        l = i*naps+k
        output = poly_fit1d(column,peaks[*,k],fitdegree,/GAUSSJ,/SILENT)
        coeff[*,l] = output
        if n_elements(WID) ne 0 then oplot, column,poly(column,output),COLOR=2

        oplot = (l eq 0) ? $
                create_struct('a'+strtrim(l,2), $
                              [[column],[peaks[*,k]],[fltarr(numstep)+1]]):$
                create_struct('a'+strtrim(l,2), $
                              [[column],[peaks[*,k]],[fltarr(numstep)+1]], $
                              oplot)      
