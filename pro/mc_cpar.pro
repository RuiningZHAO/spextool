;+
; NAME:
;     mc_cpar (Modified zparcheck from the Astronomer Users Library)
;       
; PURPOSE:
...
;
; CALLING SEQUENCE:
;     cancel = mc_cpar(progname,parameter,parname,parnum,types,dimens)
;
; INPUTS: 
...
;    
; EXAMPLE:
;     mc_cpar,'mc_meancomb',images,1,'Images',[2,3,4,5],2,CANCEl=cancel
...
;                  the dimensions.
;                                       
FUNCTION mc_cpar,progname,parameter,parnum,parname,types,dimens

  cancel = 0

  IF n_params() LT 5 THEN BEGIN
     
     print, 'Syntax - cancel = mc_cpar(progname,parameter,parnum,parname,$'
     print, '                       types,dimens)'
