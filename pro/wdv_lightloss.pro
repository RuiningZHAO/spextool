...
;
; CALLING SEQUENCE:
;     wdv_lightloss, obj, std, wguide, seeing, out, CANCEL=cancel
;
; INPUTS:
...
;-
;
pro wdv_lightloss, objfile, stdfile, wguide, seeing, outfile,CANCEL=cancel

cancel = 0

if n_params() ne 5 then begin

    print, 'Syntax - wdv_lightloss,objfile,stdfile,wguide,seeing,outfile,$'
    print, '                   CANCEL=cancel'
    cancel = 1
    return
...
cancel = mc_cpar('wdv_lightloss',objfile,1,'Objfile',7,0)
if cancel then return
cancel = mc_cpar('wdv_lightloss',stdfile,2,'Sbjfile',7,0)
if cancel then return
cancel = mc_cpar('wdv_lightloss',wguide,3,'Wguide',[2,3,4,5],0)
if cancel then return
cancel = mc_cpar('wdv_lightloss',seeing,4,'Seeing',[2,3,4,5],0)
if cancel then return
cancel = mc_cpar('wdv_lightloss',outfile,5,'Outfile',7,0)
if cancel then return
...
mc_parangle, HA_obj, DEC_obj, obslat, pa_obj, za_obj
mc_parangle, HA_std, DEC_std, obslat, pa_std, za_std
...
disp_obj = mc_atmosdisp(wobj,wguide,za_obj,press,temp,water,obsalt)
disp_std = mc_atmosdisp(wobj,wguide,za_std,press,temp,water,obsalt)
...
fracobj  = mc_slittrans(slitwd,slitht,fwhm,dx_obj,dy_obj)/ $
           mc_slittrans(slitwd,slitht,seeing,0.0,0.0)
fracstd  = mc_slittrans(slitwd,slitht,fwhm,dx_std,dy_std)/ $
           mc_slittrans(slitwd,slitht,seeing,0.0,0.0)
