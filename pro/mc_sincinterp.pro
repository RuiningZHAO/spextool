...
;
; CALLING SEQUENCE:
;     results = mc_sincinterp(xa,ya,x,CANCEL=cancel)
;
; INPUTS:
...
;
;-
function mc_sincinterp, xa,ya,x,CANCEL=cancel

  cancel = 0
  
  if n_params() ne 3 then begin
     
     print, 'Syntax - results = mc_sincinterp(xa,ya,x,CANCEL=cancel)'
     cancel = 1
     return, -1
     
  endif
  cancel = mc_cpar('mc_sincinterp',xa,'Xa',1,[2,3,4,5],1)
  if cancel then return,-1
  cancel = mc_cpar('mc_sincinterp',ya,'Ya',2,[2,3,4,5],1)
  if cancel then return,-1
  cancel = mc_cpar('mc_sincinterp',x,'X',3,[2,3,4,5],1)
  if cancel then return,-1
