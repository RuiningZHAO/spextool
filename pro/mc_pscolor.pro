...
;
; CALLING SEQUENCE:
;     result = mc_pscolor([value],BACKGROUND=background,CANCEL=cancel)
;
; INPUTS:
;     None
;
; OPTIONAL INPUTS:
;     value - The color (from mc_mkct.pro)
;
; KEYWORD PARAMETERS:
...
;
; EXAMPLE:
;     from mc_mkct.pro, 0-black, and 1-white.
;
;     if 'X' then mc_pscolor(1)=1, mc_pscolor(0)=0
;     if 'ps' then mc_pscolor(1)=0, mc_pscolor(0)=1
;
;     if 'X' then mc_pscolor(/BACKGROUND)=0
;     if 'ps' then mc_pscolor(/BACKGROUND)=1
;
; MODIFICATION HISTORY:
;     2002 - Written by M. Cushing, Institute for Astronomy, UH
;-
function mc_pscolor,value,BACKGROUND=background,CANCEL=cancel
