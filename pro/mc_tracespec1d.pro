...
;
; PURPOSE:
;     Traces spectra in an cross-dispersed image assuming column=wavelength.
;
; CATEGORY:
;     Spectroscopy
;
; CALLING SEQUENCE:
;     result = mc_tracespec1d(image,wavecal,spatcal,edgecoeffs,xranges,appos, $
...
;-
function mc_tracespec1d,image,wavecal,spatcal,edgecoeffs,xranges,orders,appos, $
                        step,sumap,winthresh,sigthresh,fitorder,WID=wid, $
                        OPLOT=oplot,CANCEL=cancel
  
  print
  print, 'Does fit wavelengths yet but does columns.'
  print
...
;     
;     cancel = 1
;     print, 'Syntax - coeff = mc_tracespec1d(image,var,edgecoeffs,norders,$'
;     print, '                                naps,slith_arc,appos,xranges,$'
;     print, '                                step,sumap,winthresh,sigthresh,$'
...
;  endif
;  
;  cancel = mc_cpar('mc_tracespec1d',image,1,'Image',[2,3,4,5],2)
;  if cancel then return,-1
;  cancel = mc_cpar('mc_tracespec1d',var,2,'Var',[2,3,4,5],2)
;  if cancel then return,-1
;  cancel = mc_cpar('mc_tracespec1d',edgecoeffs,3,'Edgecoeffs',[2,3,4,5],[2,3])
;  if cancel then return,-1
;  cancel = mc_cpar('mc_tracespec1d',norders,4,'Norders',[2,3,4,5],0)
;  if cancel then return,-1
;  cancel = mc_cpar('mc_tracespec1d',naps,5,'Naps',[2,3,4,5],0)
;  if cancel then return,-1
;  cancel = mc_cpar('mc_tracespec1d',slith_arc,6,'Slith_arc',[2,3,4,5],0)
;  if cancel then return,-1
;  cancel = mc_cpar('mc_tracespec1d',appos,7,'appos',[2,3,4,5],[1,2])
;  if cancel then return,-1
;  cancel = mc_cpar('mc_tracespec1d',xranges,8,'xranges',[2,3,4,5],[1,2])
;  if cancel then return,-1
;  cancel = mc_cpar('mc_tracespec1d',step,9,'Step',[2,3,4,5],0)
;  if cancel then return,-1
;  cancel = mc_cpar('mc_tracespec1d',sumap,10,'Sumap',[2,3,4,5],0)
;  if cancel then return,-1
;  cancel = mc_cpar('mc_tracespec1d',winthresh,11,'Win Thresh',[2,3,4,5],0)
;  if cancel then return,-1
;  cancel = mc_cpar('mc_tracespec1d',sigthresh,12,'Sig Thresh',[2,3,4,5],0)
;  if cancel then return,-1
;  cancel = mc_cpar('mc_tracespec1d',fitorder,13,'Fitorder',[2,3,4,5],0)
;  if cancel then return,-1
...
;  Make order mask for plotting if need be
     ...
     if plotpoly then begin
        ...
        for j = 0,xranges[1,i]-xranges[0,i] do begin
           ...
           omask[x[j],poly(x[j],edgecoeffs[*,0,i]): $
                 poly(x[j],edgecoeffs[*,1,i])] = 1
     ...
     arcpeaks = replicate(!values.f_nan,numstep,naps)
     pixpeaks = replicate(!values.f_nan,numstep,naps)
     ...
     for j = 0,numstep-1 do begin
        ...
        for k = 0, naps-1 do begin
           ...
           if fit[2] gt sigthresh and $
              abs(fit[1]-guessy) le winthresh then begin

              pixpeaks[j,k] = fit[1]
              linterp,yidx,yarc,fit[1],val
              arcpeaks[j,k]=val
              ...
              if n_elements(WID) ne 0 then begin
                 ...
                 wset, wid                 
                 plots, column[j],fit[1],PSYM=1,COLOR=3
           ...
           if plotline then begin
              ...
              plot, yidx,zimg,/XSTY,/YSTY,PSYM=10,$
                    ...
                    ((finite(arcpeaks[j,k]) eq 1) ? 'Good':'Bad')
...
;  Get the fit plot window ready if need be.
     ...
     if plotpoly then begin
        ...
        plot,[1],[1],XRANGE=[min(wavecal[orderpixels],MAX=max,/NAN),max],$
             ...
             /XSTY,/YSTY,TITLE='Order '+strtrim(orders[i],2)
...
;  Fit the trace in w/s space
        ...
        output = robustpoly1d(column,arcpeaks[*,k],fitorder,3,0.01, $
...
;  Plot it if requested.
        ...
        if plotpoly then begin
           ...
           oplot,column,arcpeaks[*,k],PSYM=4
           oplot,column,poly(column,output),COLOR=2
           bad = where(goodbad eq 0,cnt)
           if cnt ne 0 then oplot,column[bad],arcpeaks[bad,k],PSYM=4,COLOR=4
