...
pro xmergexd_combinespec,state
  ...
  mc_mergespec,bspec_w,double(bspec_f),gspec_w,double(gspec_f),owave,oflux, $
               ...
               OERROR=oerror,OBITFLAG=obitflag,CANCEL=cancel
...
shortxd = mc_cfld(state.w.shortxd_fld,7,/EMPTY,CANCEL=cancel)
if cancel then return
shortxd = mc_cfile(shortxd,WIDGET_ID=state.w.xmergexd_base,CANCEL=cancel)
if cancel then return

longxd = mc_cfld(state.w.longxd_fld,7,/EMPTY,CANCEL=cancel)
if cancel then return
longxd = mc_cfile(longxd,WIDGET_ID=state.w.xmergexd_base,CANCEL=cancel)
if cancel then return

;  Read spectra.

mc_readspec,shortxd,shortxd,shdr,obsmode,start,stop,norders,naps,orders,$
            xunits,yunits,slith_pix,slith_arc,slitw_pix,slitw_arc,rp,$
            airmass,xtitle,ytitle,CANCEL=cancel
if cancel then return
shortxd = mc_caaspecflags(shortxd,CANCEL=cancel)
if cancel then return

mc_readspec,longxd,longxd,lhdr,obsmode,start,stop,norders,naps,orders,$
            dxunits,dyunits,slith_pix,slith_arc,slitw_pix,slitw_arc,rp,$
            airmass,CANCEL=cancel
if cancel then return
longxd = mc_caaspecflags(longxd,CANCEL=cancel)
if cancel then return
...
if state.p.plottype eq 'Overlap' then begin
    ...
    plot, findgen(10),/XSTY,/YSTY,/NODATA,$
          ...
          TITLE='!5Signal-to-Noise Spectra',CHARSIZE=1.5
    ...
    plot, (*state.d.tempmergedspec)[*,0], $
          ...
          TITLE='!5Signal-to-Noise Spectrum',CHARSIZE=1.5
...
if state.p.plottype eq 'Overlap' then begin
    ...
    plot, findgen(10),/XSTY,/YSTY,/NODATA,$
          ...
          TITLE='!5Overlapping Spectra',CHARSIZE=1.5
    ...
    plot, (*state.d.tempmergedspec)[*,0],(*state.d.tempmergedspec)[*,1],$
      ...
      TITLE='!5Merged Spectrum',CHARSIZE=1.5
...
pro xmergexd_writefile,state
  ...
  path = file_dirname(shortxd,/MARK)
  shortxd = file_basename(shortxd)
  longxd = file_basename(longxd)
 
  ofile = mc_cfld(state.w.oname_fld,7,/EMPTY,CANCEL=cancel)
  if cancel then return
  
  scale = mc_cfld(state.w.scale_fld,4,/EMPTY,CANCEL=cancel)
  if cancel then return
  ...
  sxdobs = fxpar(*state.d.shortxdhdr,'DATE_OBS')
  lxdobs = fxpar(*state.d.longxdhdr,'DATE_OBS')
  ...
  fxaddpar,hdr,'SXDOBS',sxdobs,' UT date of SXD observation'
  fxaddpar,hdr,'LXDOBS',lxdobs,' UT date of LXD observation'
  ...
  hdr = mc_addhistlabel(hdr,'Xmergexd History',CANCEL=cancel)
  if cancel then return
...
;  Get the ShortXD header
  ...
  hdr = mc_addhistlabel(hdr,'ShortXD History',CANCEL=cancel)
  if cancel then return

  sxdhist = fxpar(*state.d.shortxdhdr,'HISTORY')
  sxaddhist,sxdhist,hdr
  
  hdr = mc_addhistlabel(hdr,'LongXD History',CANCEL=cancel)
  if cancel then return

  lxdhist = fxpar(*state.d.longxdhdr,'HISTORY')
  sxaddhist,lxdhist,hdr
  
  writefits,path+ofile+'.fits',*state.d.mergedspec,hdr
  xvspec,path+ofile+'.fits',/PLOTLINMAX,/PLOTFIX
...
case uvalue of
    ...
    'LongXD': begin
        ...
        mc_setfocus,state.w.longxd_fld
    ...
    'ShortXD': begin
        ...
        mc_setfocus,state.w.shortxd_fld
...
title = 'Xmergexd '+version+' for '+instr.instr

state.w.xmergexd_base = widget_base(TITLE=title, $
                                      /COLUMN,$
                                      /TLB_SIZE_EVENTS)
         ...
         box1_base = widget_base(col1_base,$
            ...
            row = widget_base(box1_base,$
               ...
               input_fld = coyote_field2(row,$
                                         ...
                                         XSIZE=18,$
                                         VALUE='v900_mon_sxd_xcleanspec.fits',$
                                         EVENT_PRO='xmergexd_event',$
                                         /CR_ONLY,$
                                         TEXTID=textid)
            ...
            row = widget_base(box1_base,$
               ...
               input_fld = coyote_field2(row,$
                                         ...
                                         XSIZE=18,$
                                         EVENT_PRO='xmergexd_event',$
                                         VALUE='v900_mon_lxd_xcleanspec.fits',$
                                         /CR_ONLY,$
                                         TEXTID=textid)
...
; Get things running.  Center the widget using the Fanning routine.
    ...
    cgcentertlb,state.w.xmergexd_base
