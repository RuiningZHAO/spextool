...
;
; CALLING SEQUENCE:
;    xmc_conkern,std_wave,std_flux,wvega,fvega,fcvega,fc2vega,awave,atrans,$
...
p = {absxrange1:[min(std_wave[idx],max=max),max],$
     absxrange2:[min(std_wave[idx],max=max),max],$
...
mc_vegacorr,state.d.wave[z],state.d.nflux[z],state.d.wvega,$
...
wave = state.d.wave
nflux = state.d.nflux
fcvega = state.d.fcvega
linereg = state.r.linereg

save,FILENAME='stuff.sav',wave,nflux,fcvega,fnvega,linereg,wvega

mc_vegaconv,state.d.wave,state.d.nflux,wvega,state.d.fcvega,$
...
;******************************************************************************
;
pro xmc_conkern_zoom,IN=in,OUT=out
...
case uvalue of
    ...
    'Construct Kernel': xmc_conkern_conkernel
    ...
    'Keyboard': begin
        ...
        case strtrim(event.ch,2) of 
            ...
            'i': xmc_conkern_zoom, /IN

            'o': xmc_conkern_zoom, /OUT
    ...
    'Plot Window': begin
        ...
        if event.value eq 'Raw' then begin
            ...
            state.p.activewin = 1
            xmc_conkern_plotupdate
            widget_control, state.w.plotwin1, $
                            EVENT_PRO='xmc_conkern_plotwinevent1'
            ...
            xmc_conkern_plotupdate
            widget_control, state.w.plotwin1, $
                            EVENT_PRO='xmc_conkern_plotwinevent2'
    ...
    'Retry': begin
        ...
        xmc_conkern_conkernel

    end

;    'TWF Field': begin

;        val = mc_cfld(state.w.twf_fld,4,/EMPTY,CANCEL=cancel)
;        if cancel then return
;        state.r.twf = val
;        xmc_conkern_conkernel        
...
;******************************************************************************
;
pro xmc_conkern_plotwinevent1, event
...
;******************************************************************************
;
pro xmc_conkern_plotwinevent2, event
...
;******************************************************************************
;
pro xmc_conkern_resize,event

common xmc_conkern_state

widget_control, state.w.xmc_conkern_base, TLB_GET_SIZE = size
...
;******************************************************************************
;
pro xmc_conkern,std_wave,std_flux,wvega,fvega,fcvega,fc2vega,awave,$
...
if n_params() lt 8 then begin
    ...
    print, 'Syntax - xmc_conkern,std_wave,std_flux,wvega,fvega,fcvega,fc2vega,$'
...
cancel = mc_cpar('xmc_conkern',std_wave,1,'Std_wave',[2,3,4,5],1)
if cancel then return
cancel = mc_cpar('xmc_conkern',std_flux,2,'Std_flux',[2,3,4,5],1)
if cancel then return
cancel = mc_cpar('xmc_conkern',wvega,3,'Wvega',[2,3,4,5],1)
if cancel then return
cancel = mc_cpar('xmc_conkern',fvega,4,'Fvega',[2,3,4,5],1)
if cancel then return
cancel = mc_cpar('xmc_conkern',fcvega,5,'Fcvega',[2,3,4,5],1)
if cancel then return
cancel = mc_cpar('xmc_conkern',fc2vega,6,'Fc2vega',[2,3,4,5],1)
if cancel then return
cancel = mc_cpar('xmc_conkern',awave,7,'Awave',[2,3,4,5],1)
if cancel then return
cancel = mc_cpar('xmc_conkern',atrans,8,'Atrans',[2,3,4,5],1)
if cancel then return

common xmc_conkern_state

if not xregistered('xmc_conkern') then begin
        
    xmc_conkern_initcommon,std_wave,std_flux,wvega,fvega,fcvega,fc2vega,$
      awave,atrans,XTITLE=xtitle,YTITLE=ytitle
...
;  Build the widget.
    ...
    state.w.xmc_conkern_base = widget_base(TITLE='Xmc_Conkern', $
                                         /COLUMN,$
                                         /TLB_SIZE_EVENTS)
    
       quit_button = widget_button(state.w.xmc_conkern_base,$
                                   FONT=buttonfont,$
                                   EVENT_PRO='xmc_conkern_event',$
                                   VALUE='Cancel',$
                                   UVALUE='Cancel')
       
       state.w.keyboard = widget_text(state.w.xmc_conkern_base, $
                                      /ALL_EVENTS, $
                                      SCR_XSIZE=1, $
                                      SCR_YSIZE=1, $
                                      UVALUE='Keyboard', $
       ...
       state.w.message = widget_text(state.w.xmc_conkern_base, $
                                     YSIZE=1)

       row_base = widget_base(state.w.xmc_conkern_base,$
                              /ROW,$
                              FRAME=2,$
                              EVENT_PRO='xmc_conkern_event',$
                              /BASE_ALIGN_CENTER)
       ...
       state.w.plotwin1 = widget_draw(state.w.xmc_conkern_base,$
...
;          state.w.dlambda_fld = [dlambda_fld,textid]
       ...
       state.w.plotwin2 = widget_draw(state.w.xmc_conkern_base,$
                                      XSIZE=state.p.plot2size[0],$
                                      YSIZE=state.p.plot2size[1],$
                                      UVALUE='Plot Window 2')
       
       button = widget_button(state.w.xmc_conkern_base,$
                              FONT=buttonfont,$
                              EVENT_PRO='xmc_conkern_event',$
                              VALUE='Accept',$
                              UVALUE='Accept')
...
; Get things running.  Center the widget using the Fanning routine.
    ...
    cgcentertlb,state.w.xmc_conkern_base
    widget_control, state.w.xmc_conkern_base, /REALIZE
...
;  Get sizes for things.
    ...
    widget_geom = widget_info(state.w.xmc_conkern_base, /GEOMETRY)
...
; Start the Event Loop. This will be a non-blocking program.
    ...
    XManager, 'xmc_conkern', $
      state.w.xmc_conkern_base,$
      EVENT_HANDLER='xmc_conkern_resize'
