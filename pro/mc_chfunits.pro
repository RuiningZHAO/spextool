...
function mc_chfunits,wave,flux,iwunit,ifunit,ofunit,IERROR=ierror,$
  ...
  cancel = mc_cpar('mc_chfunits',wave,1,'Wave',[2,3,4,5],[0,1,2])
  if cancel then return,-1
  cancel = mc_cpar('mc_chfunits',flux,2,'Flux',[2,3,4,5],[0,1,2])
  if cancel then return,-1
  cancel = mc_cpar('mc_chfunits',iwunit,3,'Iwunit',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_chfunits',ifunit,4,'Ifunit',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_chfunits',ofunit,5,'Ofunit',[2,3,4,5],0)
  if cancel then return,-1
  
  if n_elements(IERROR) ne 0 then begin
     
     cancel = mc_cpar('mc_chfunits',ierror,6,'IERROR',[2,3,4,5],[0,1,2])
     if cancel then return,-1
     doerr = 1
