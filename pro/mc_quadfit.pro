...
;
; CALLING SEQUENCE:
;     result = mc_quadfit(img,ERR=err,JUSTFIT=justfit,SILENT=silent,$
...
;     2009-03-11 - Renanmed mc_quadfit
;-
function mc_quadfit,img,ERR=err,JUSTFIT=justfit,SILENT=silent,$
                    DOALPHA=doalpha,COVAR=covar,FIT=fit,RMS=rms,$
                    CHISQ=chisq,RCHISQ=rchisq,Q=q,SIGMA=sigma,$
                    CANCEL=cancel
...
;  Check parameters
  ...
  if n_params() lt 1 then begin
     ...
     print, 'Syntax -  result = mc_quadfit(img,ERR=err,JUSTFIT=justfit,$'
  ...
  cancel = mc_cpar('mc_quadfit',img,1,'Img',[2,3,4,5],2)
...
;  Solve the normal equations.
  ...
  mc_gaussj,alpha,beta,inverted=covar,solution=coeffs,CANCEL=cancel
  if cancel then return,-1
     ...
     if not keyword_set(SILENT) then begin
        ...
        for i = 0, ncoeffs-1 do begin
           ...
           string = 'Coeff #'+string(i,format='(i2.2)')+'   =  '+ $
                    strcompress(coeffs[i],/re)+'  +/-  '+ $
                    strcompress(sigma[i], /re)
           print, string
