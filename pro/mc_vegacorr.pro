...
;
; CALLING SEQUENCE:
;     mc_vegacorr,wave,flux,wvin,fvin,fcvin,vshift,z,WMIN=wmin,$
...
;                  Astronomy, UH
;-
pro mc_vegacorr, wave,flux,wvin,fvin,fcvin,vshift,z,WMIN=wmin,WMAX=wmax,$
                 PLOTCORR=plotcorr,CANCDL=cancel

  cancel = 0
  
  if n_params() lt 5 then begin
     
     print, 'Syntax - mc_vegacorr,wave,flux,wvin,fvin,fcvin,vshift,z,$'
  ...
  cancel = mc_cpar('mc_vegacorr',wave,1,'Wave',[2,3,4,5],1)
  if cancel then return
  cancel = mc_cpar('mc_vegacorr',flux,2,'Flux',[2,3,4,5],1)
  if cancel then return
  cancel = mc_cpar('mc_vegacorr',wvin,3,'Wvin',[2,3,4,5],1)
  if cancel then return
  cancel = mc_cpar('mc_vegacorr',fvin,4,'Fvin',[2,3,4,5],1)
  if cancel then return
  cancel = mc_cpar('mc_vegacorr',fcvin,5,'Fcvin',[2,3,4,5],1)
  if cancel then return
...
; --- Check number of points
  ...
  if (nvega lt ndata) then print, 'Warning: Input data has higher ' + $
                                  'resolution than Vega model!'
  num   = max([ndata,nvega])
...
; --- Plot cross correlation function versus lag
  ...
  if keyword_set(PLOTCORR) then begin
     ...
     plot, lag, result, xtitle='Lag', ytitle='Cross Correlation',$
           psym=-4,/xsty,/ysty
     plots,[lshift,lshift],!y.mc_crange,color=2
