...
;
; CALLING SEQUENCE:
;     result = mc_fsextract(string,INDEX=index,FILENAME=filename,NFILES=nfiles,$
...
;     2015-01-13 - Added the WIDGET_ID keyword.
;-
function mc_fsextract,string,INDEX=index,FILENAME=filename,NFILES=nfiles,$
                      WIDGET_ID=widget_id,CANCEL=cancel
...
if n_params() lt 1 then begin
    ...
    cancel = 1
    print, 'Syntax -  result = mc_fsextract(string,INDEX=index,$'
    print, '                                FILENAME=filename,NFILES=nfiles,$'
    print, '                                CANCEL=cancel)'
    return, -1
    
endif
cancel = mc_cpar('mc_fsextract',string,1,'String',7,0)
...
if keyword_set(INDEX) then begin
    ...
    for i = 0, n_elements(comma_parse) - 1 do begin
        ...
        message = 'Type conversion error.  Probably an INDEX versus FILENAME'+$
                  ' File Read Mode problem.'
