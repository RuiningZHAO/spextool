...
;     bits  - A scalar or vector of bits to search.  Note that the
;             "first" bit is denoted as zero, while the "second" bit is
;             denoted as 1.  
;
; OPTIONAL INPUTS:
;     None
...
function mc_bitset,array,bits,CANCEL=cancel
  ...
  cancel = mc_cpar('mc_bitset',array,1,'Array',[1,2,3])
  if cancel then return,-1
  cancel = mc_cpar('mc_bitset',bits,2,'Bits',[1,2,3,12,13])
  if cancel then return,-1
