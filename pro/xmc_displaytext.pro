...
;******************************************************************************
;
pro xmc_displaytext,input,TITLE=title,WSIZE=wsize,POSITION=position,$
  ...
  cancel = mc_cpar('xmc_displaytext',input,1,'Input',[7],[0,1,2])
...
;  Check to see an xmc_displaytext is already active.

  nwin = xregistered('xmc_displaytext')+1
  if n_elements(title) ne 0 then title = strtrim(nwin,2)+' - '+title
...
;  Create the widget
  ...
  cgcentertlb,displaytext_base,position[0],position[1]
