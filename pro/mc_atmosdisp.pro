...
;
; CALLING SEQUENCE:
;     result = mc_atmosdisp(wave,wave_0,za,pressure,temp,[water],[obsalt],$
;                           CANCEL=cancel)
;
; INPUTS:
...
;     obalt    - The observatory latitude in km.
;
; KEYWORD PARAMETERS:
...
;
;-
function mc_atmosdisp,wave,wave_0,za,pressure,temp,water,obsalt,CANCEL=cancel

  cancel = 0
  
  if n_params() lt 5 then begin
     
     print, 'Syntax - result = mc_atmosdisp(wave,wave_0,za,pressure,temp,$'
  ...
  cancel = mc_cpar('mc_atmosdisp',wave,1,'Wave',[2,3,4,5],[0,1])
  if cancel then return,-1
  cancel = mc_cpar('mc_atmosdisp',wave_0,2,'Wave_0',[2,3,4,5],[0,1])
  if cancel then return,-1
  cancel = mc_cpar('mc_atmosdisp',za,3,'ZA',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_atmosdisp',pressure,4,'Pressure',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_atmosdisp',temp,5,'Temp',[2,3,4,5],0)
  if cancel then return,-1
  
  if n_params() eq 6 then cancel=mc_cpar('mc_atmosdisp',water,6,'Water', $
                                      [2,3,4,5],0)
  if cancel then return,-1
  
  if n_params() eq 7 then cancel=mc_cpar('mc_atmosdisp',obsalt,7,'Obslat', $
                                      [2,3,4,5],0)
  if cancel then return,-1 
...
; Compute index of refraction
  ...
  nindx  = mc_nlambda(wave,pressure,temp,water)
  nindx0 = mc_nlambda(wave_0,pressure,temp,water)
