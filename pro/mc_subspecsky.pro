...
;
; CALLING SEQUENCE:
;     mc_subspecsky,images,var,edgecoeffs,norders,ybuffer,xranges,$
...
;-
pro mc_subspecsky,images,var,edgecoeffs,norders,ybuffer,xranges,$
...
if n_params() lt 6 then begin
    ...
    cancel = 1
    print, 'Syntax - mc_subspecsky,images,var,edgecoeffs,norders,ybuffer,$'
...
cancel = mc_cpar('mc_subspecsky',images,1,'Images',[2,3,4,5],[2,3])
if cancel then return
cancel = mc_cpar('mc_subspecsky',var,2,'Var',[2,3,4,5],[2,3])
if cancel then return
cancel = mc_cpar('mc_subspecsky',edgecoeffs,3,'Edgecoeffs',[2,3,4,5],[2,3])
if cancel then return
cancel = mc_cpar('mc_subspecsky',norders,4,'Norders',[2,3,4,5],0)
if cancel then return
cancel = mc_cpar('mc_subspecsky',ybuffer,5,'Ybuffer',[2,3,4,5],0)
if cancel then return
cancel = mc_cpar('mc_subspecsky',xranges,6,'Xranges',[2,3,4,5],[1,2])
if cancel then return
...
;  Compute background of the pixels in the slit and subtract.
            ...
            if n_elements(DEG) eq 0 then begin
                ...
                z = where(mc_findoutliers(slit_flux,3) eq 1,count)
