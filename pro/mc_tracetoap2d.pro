...
pro mc_tracetoap2d,omask,wavecal,spatcal,tri,tracecoeffs,naps,orders,doorders, $
  ...
  if cancel then return
  cancel = mc_cpar('mc_tracetoap2d',doorders,8,'doorders',[2,3],[1])
  if cancel then return
...
;  Get order numbers
  ...
  for i = 0,norders-1 do begin
     ...
     for j = 0,naps-1 do begin
        ...
        appos[j,i] = mean(trace_ap)

        ix = griddata(wavecal[z],spatcal[z],xx[z],/LINEAR,TRIANGLES=tri.(i),$
                      XOUT=wave,YOUT=trace_ap,MISSING=!values.f_nan)
        
        iy = griddata(wavecal[z],spatcal[z],yy[z],/LINEAR,TRIANGLES=tri.(i), $
                      XOUT=wave,YOUT=trace_ap,MISSING=!values.f_nan)
        
        
        array = [[ix[*,0]],[iy[*,0]]]
;        forprint, ix[*,0],iy[*,0],TEXT=2
;        re = ' '
;        read, re
