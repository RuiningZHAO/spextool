...
;
; CALLING SEQUENCE:
;     result = mc_fiterpolate(image,ncg,nrg,CANCEL=cancel)
;
; INPUTS:
...
;     2009-03-11 - Renamed mc_fiterpolate
;-
function mc_fiterpolate,image,ncg,nrg,CANCEL=cancel
...
;  Check parameters
  ...
  if n_params() lt 3 then begin
     ...
     print, 'Syntax - result = mc_fiterpolate(image,ncg,nrg,CANCEL=cancel)'
     cancel = 1
     return, -1
     
  endif
  cancel = mc_cpar('mc_fiterpolate',image,1,'Image',[2,3,4,5],2)
  if cancel then return,-1
  cancel = mc_cpar('mc_fiterpolate',ncg,2,'Ncg',[2,3,4,5],0)
  if cancel then return,-1
  cancel = mc_cpar('mc_fiterpolate',nrg,3,'Nrg',[2,3,4,5],0)
  if cancel then return,-1
...
;  Determine the values z,dy1,dy2, and dy12, at the grid points.
  ...
  for i = 0, nxgrid-1 do begin
     ...
     for j = 0,nygrid-1 do begin
        ...
        c = mc_quadfit(image[x1:x2,y1:y2],/JUSTFIT)
  ...
  for i = 0, ncg-1 do begin
     ...
     for j = 0,nrg-1 do begin
        ...
        fitimg[gridinfo[idx].xrange[0]:gridinfo[idx].xrange[1],$
           ...
           mc_bicuval(reform(vals[0,id]),reform(vals[1,id]),$
