...
;                 rp,airmass,xtitle,ytitle,instr,SILENT=silent,HISTORY=history,$
...
;     HISTORY   - A string given the HISTORY.
;     WIDGET_ID - If given, any message will be given by a
;                 pop-up over the widget given by WIDGET_ID.
...
;        2014-05-14 - Added the HISTORY keyword.
;        2014-07-29 - Added WIDGET_ID keyword.
;        2015-03-06 - Added a catch to deal with the fact that the
;                     airmass keyword for SpeX changed to TCS_AM.  
;-

pro mc_readspec,specname,spc,hdr,obsmode,start,stop,norders,naps,orders,$
                xunits,yunits,slith_pix,slith_arc,slitw_pix,slitw_arc,$
                rp,airmass,xtitle,ytitle,instr,HISTORY=history,$
                SILENT=silent,WIDGET_ID=widget_id,CANCEL=cancel
...
;  Check parameters
  ...
  if n_params() lt 1 then begin
     ...
     print, '                     HISTORY=history,WIDGET_ID=widget_id,CANCEL=cancel'
     cancel = 1
     return
     
  endif
  cancel = mc_cpar('mc_readspec',specname,1,'Specname',7,0)
  if cancel then return
  
  spc = readfits(specname,hdr,/SILENT,EXTEN=0)
  
;  Get hdr info.
  
  naps     = fxpar(hdr,'NAPS')
  if naps eq 0 then begin
     
     message = 'This FITS file is not a Spextool FITS file.'
  ...
  if airmass eq 0 then airmass = fxpar(hdr,'TCS_AM')
  
  instr     = fxpar(hdr,'INSTR')

  history = sxpar(hdr,'HISTORY')
