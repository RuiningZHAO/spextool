...
;
; CALLING SEQUENCE:
;     results = mc_roundgt(val,CANCEL=cancel)
;    
; INPUTS:
...
;-
function mc_roundgt,val,CANCEL=cancel

  cancel = 0
  
  if n_params() ne 1 then begin
     
     print, 'Syntax - mc_roundgt(val,CANCEL=cancel)'
     cancel = 1
     return,-1
     
  endif
  cancel = mc_cpar('mc_roundgt',val,1,'Val',[2,3,4,5],[0,1,2,3])
