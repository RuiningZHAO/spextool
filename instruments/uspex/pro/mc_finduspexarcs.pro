...
;
; PURPOSE:
;     Finds the arc images in a list of uSpeX data.
;
; CATEGORY:
;     Spectroscopy
;
; CALLING SEQUENCE:
;     result = mc_finduspexarcs(files,MASK=mask,AB=ab,CANCEL=cancel)
;
; INPUTS:
...
;     ARG_LAMP,CALMIR,QTH_LAMP,INC_LAMP,IR_LAMP, and BEAM
;     
; EXAMPLE:
;     
;
; MODIFICATION HISTORY:
;     2014-06-24 - modified version of mc_findspexarcs.pro.
;-
function mc_finduspexarcs,files,MASK=mask,AB=ab,CANCEL=cancel
...
;  Check parameters
  ...
  if n_params() lt 1 then begin
     ...
     print, 'Syntax -  result = mc_findspexarcs(files,MASK=mask,AB=ab,$'
     print, '                                   CANCEL=cancel)'
     cancel = 1
     return, -1
     
  endif
  cancel = mc_cpar('mc_findspexarcs',files,1,'Files',7,[0,1])
  ...
  for i = 0, n-1 do begin
     ...
     ARG    = strtrim(fxpar(hdr,'ARG_LAMP'),2)
  ...
  if n_elements(arc) eq 0 then begin

     cancel = 1
     return, -1

  endif else return, arc
