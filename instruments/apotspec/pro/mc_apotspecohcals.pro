...
;  Read sky frames
  ...
  mc_readapotspecfits,armc_cfiles,data,hdrinfo,var,ROTATE=rotation, $
...
; Display image to shows points on
  ...
  ximgtool,arc,WID=wid,GROUP_LEADER=state.w.xspextool_base,RANGE='ZScale', $
...
;  Load the files
  ...
  if strtrim(dark,2) ne '' then $
     tmp = mc_cfile(state.r.calpath+dark,WIDGET_ID=state.w.xspextool_base,$
...
;  Read info from cal file
  ...
  slitw_pix = w.slitw_arc/modeinfo.ps
...
;  Write the results to disk.
  ...
  mc_writeflat,mc_unrotate(mean,modeinfo.rotation),$
               ...
               slitw_pix,w.slitw_arc,modeinfo.modename,rms,resolvingpower, $
...
pro tspeccals_event,event
  ...
  case uvalue of 
     ...
     'Slit Width': w.slitw_arc = strtrim(w.slits[event.index],2)
...
;============================================================================
;
pro mc_apotspecohcals,parent,FAB=fab,CANCEL=cancel
...
;  Create the widget
  ...
  col1_base = widget_base(base,$
     ...
     dl = widget_droplist(col1_base,$
                          FONT=buttonfont,$
                          TITLE='Slit Width (arcsec):',$
                          VALUE=w.slits,$
                          UVALUE='Slit Width')
