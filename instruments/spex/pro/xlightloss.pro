...
case uvalue of
    ...
    'Help': xmc_displaytext,filepath('xlightloss_helpfile.txt',$
                                     ROOT_DIR=state.w.packagepath, $
                                     SUBDIR='helpfiles'), $
                            TITLE='Xlightloss Help File', $
                            GROUP_LEADER=state.w.xlightloss_base,WSIZE=[600,400]
...
std = mc_cfld(state.w.std_fld,7,/EMPTY,CANCEL=cancel)
if cancel then return

obj = mc_cfld(state.w.obj_fld,7,/EMPTY,CANCEL=cancel)
if cancel then return

guide = mc_cfld(state.w.guide_fld,4,/EMPTY,CANCEL=cancel)
if cancel then return

seeing = mc_cfld(state.w.seeing_fld,4,/EMPTY,CANCEL=cancel)
if cancel then return

ofile = mc_cfld(state.w.oname_fld,7,/EMPTY,CANCEL=cancel)
if cancel then return

wdv_lightloss,obj,std,guide,seeing,ofile
...
cgcentertlb,state.w.xlightloss_base
