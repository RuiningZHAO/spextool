...
function tspec_clean, exposure, header, variance=variance, sigma=sigma, _extra=extra
...
; NAN bad pixels
  if keyword_set(sigma) then begin
     ...
     image[where(image ne filtered)] = !values.d_nan
  endif
  
; NAN saturating pixels
;  saturating = where(image ge 25000d, saturating_count)
;  if saturating_count ne 0 then image[saturating] = !values.d_nan
